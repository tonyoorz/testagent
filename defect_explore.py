...
# 缓存版本号 - 修改此值可使缓存失效
_DATA_CACHE_VERSION = "v3_lazy_year_loading"
...
def _cached_load_master_data(years_key=None, cache_version=None):
    ...
    for year in years:
        ...
        if not os.path.exists(master_file_path):
            print(f"⚠️ 未找到 {year} 年 master 文件: {master_file_path}")
            continue
...
try:
    ...
    if IS_RELOADER:
        ...
        print("🚀 主进程开始数据加载...")
        df, test_df = load_application_data(initial_years=(DEFAULT_DEFECT_YEAR,))
...
@performance_monitor
def filter_dataframe(source_df, projects=None, start_date=None, end_date=None, aidas=None, statuses=None, pus=None, testers=None, fvs=None, ecus=None, lead_models=None, fvps=None, markets=None):
    """通用数据筛选函数"""
    ensure_defect_data_for_date_range(start_date, end_date)
    filtered_df = globals().get('df', source_df).copy()
...
def create_long_runner_analysis_content(active_label_style):
    ...
    def load_defect_info():
        """从defect数据文件加载ticket名称和tester信息（支持多年份）"""
        defect_info = {}
        import glob
        defect_files = glob.glob('defect/*_defect.json')
