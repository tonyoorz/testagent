...
# 使用 Waitress 启动 8051 服务
python serve_waitress.py --host 0.0.0.0 --port 8051 --threads 8
```

说明：
- `serve_waitress.py` 会自动关闭 Dash 调试模式和 reloader，适合 Windows 内网多用户访问。
- `--threads 8` 是较稳妥的默认值，建议先从 8 开始，根据压测结果再调整。
- 如果存在 AI 聊天和 Excel 导出高峰，建议先控制同时使用人数，避免长任务挤占交互请求。
