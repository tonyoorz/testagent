...
def register_test_coverage_callbacks(app, prefix="de-tc"):
    ...
    @app.callback(
        ...
        [Input(f'{prefix}-year-dropdown', 'value'),
    ...
    def update_test_coverage_charts(selected_years, selected_projects, selected_testweeks, selected_pus,
        ...
        try:
            ...
            # 年份筛选
            if is_filter_active(selected_years):
                year_series = pd.to_numeric(filtered_tdf['year'], errors='coerce') if 'year' in filtered_tdf.columns else pd.Series(dtype='float64')
                if year_series.dropna().empty and 'test_week' in filtered_tdf.columns:
                    extracted_years = pd.to_numeric(filtered_tdf['test_week'].astype(str).str.extract(r'(\d{2,4})\s*-\s*CW')[0], errors='coerce')
                    extracted_years = extracted_years.apply(lambda y: y + 2000 if pd.notna(y) and y < 100 else y)
                    year_series = extracted_years
                if not year_series.dropna().empty:
                    filtered_tdf = filtered_tdf[year_series.astype('Int64').isin([int(y) for y in selected_years])]
            ...
            # 判断是否有活跃的筛选器（非默认值）
            has_active_filters = (
                is_filter_active(selected_years) or
