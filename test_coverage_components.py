...
def create_test_coverage_filters(tdf, prefix="tc"):
    ...
    year_series = pd.to_numeric(tdf['year'], errors='coerce') if 'year' in tdf.columns else pd.Series(dtype='float64')
    if year_series.dropna().empty and 'test_week' in tdf.columns:
    ...
    years = sorted({int(v) for v in year_series.dropna().tolist()})
    year_default = [2026] if 2026 in years else []
    # lead_models 列表获取已移除
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.Label('年份:', style=active_label_style),
                dcc.Dropdown(
...
def create_test_coverage_charts(prefix="tc"):
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.H4("图表 3: 按测试用例和测试周分类的状态", style=active_label_style),
                dcc.Loading(
                    type='circle',
                    children=html.Div([
