...
def get_authenticated_session(auth_method_choice, sso_login_file=None, cookie_file_path=None):
    ...
    if auth_method_choice == 'sso':
        ...
        actual_cookie_file = cookie_file_path or "cookie.txt"
        try:
            logging.info(f"尝试从 '{actual_cookie_file}' 文件读取 Cookie...")
            if os.path.exists(actual_cookie_file):
                with open(actual_cookie_file, "r", encoding="utf-8") as f:
                    cookie_val = f.read().strip()
                if not cookie_val:
                    logging.error(f"Cookie 文件 '{actual_cookie_file}' 为空."); return None
            else:
                logging.warning(f"Cookie 文件 '{actual_cookie_file}' 未找到，退回到预定义 DEFAULT_COOKIE.")
                cookie_val = DEFAULT_COOKIE

            if not cookie_val:
                logging.error("没有可用的 Cookie 值，请检查文件或 DEFAULT_COOKIE"); return None

            session_obj.headers.update({
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36",
                "cookie": cookie_val
            })
            logging.info(f"使用 Cookie (来自 {'文件' if os.path.exists(actual_cookie_file) else '预定义值'}) 认证.")
        except Exception as e:
...
def get_single_defect_history_module(defect_id_hist, session_hist, headers=None):
    # history API lives under the same workspace path as defects
    ...
    try:
        # ``headers`` may be provided for explicit cookie/UA as in history_downloader
        resp_hist = session_hist.get(
            history_api_url_actual,
            params=request_params_hist,
            headers=headers,
            verify=False,
        ...
        if not (200 <= resp_hist.status_code < 300):
            if resp_hist.status_code == 403:
                logging.warning(f"权限不足，无法获取Defect ID {defect_id_hist}历史 (403). URL: {resp_hist.url}")
            else:
...
def fetch_defect_histories_parallel(defect_ids_list_hist, session_obj_hist, max_workers_hist, hist_output_dir_actual, save_csv_hist=False):
    processed_count = 0
    error_count = 0
    # match downloader7 behavior: save files directly under the provided output dir
    os.makedirs(hist_output_dir_actual, exist_ok=True)
    logging.info(f"单个defect原始历史将保存到: {hist_output_dir_actual}")

    # capture headers from session for history calls
    headers_for_hist = dict(session_obj_hist.headers) if hasattr(session_obj_hist, 'headers') else None
    ...
    with tqdm(total=len(defect_ids_list_hist), desc="获取Defect历史") as pbar_hist:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers_hist) as executor_hist:
            future_to_id_hist = {
                executor_hist.submit(get_single_defect_history_module, df_id, session_obj_hist, headers_for_hist): df_id
            ...
            for future_hist in concurrent.futures.as_completed(future_to_id_hist):
                ...
                try:
                    ...
                    if raw_history_data and raw_history_data.get("total_count", 0) > 0:
                        history_filename = os.path.join(hist_output_dir_actual, f"{defect_id_current}_history.json")
                        # write manually to avoid excessive info-level logging from save_data
                        try:
                            with open(history_filename, "w", encoding="utf-8") as hf:
                                json.dump(raw_history_data, hf, ensure_ascii=False, indent=2)
                        except Exception as e_write:
                            logging.error(f"保存历史文件 {history_filename} 时出错: {e_write}")
                        processed_count += 1
...
def fetch_defect_histories_parallel_to_db(defect_ids_list_hist, session_obj_hist, max_workers_hist, store_obj, team_name, save_files_dir=None, save_csv_hist=False):
    ...
    # if user requested file output, mimic downloader7 and save directly under save_files_dir
    if save_files_dir:
        os.makedirs(save_files_dir, exist_ok=True)
        logging.info(f"单个defect原始历史将保存到: {save_files_dir}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers_hist) as executor_hist:
            ...
            for future_hist in concurrent.futures.as_completed(future_to_id_hist):
                ...
                try:
                    ...
                    if save_files_dir:
                        history_filename = os.path.join(save_files_dir, f"{defect_id_current}_history.json")
                        try:
                            with open(history_filename, "w", encoding="utf-8") as hf:
                                json.dump(raw_history_data, hf, ensure_ascii=False, indent=2)
                        except Exception as e_write:
...
def main():
    ...
    # 默认启用历史下载，与 downloader7 保持一致
    history_group.add_argument("--fetch-history", action='store_true', default=True, help="启用 Defect History 下载 (当年至今, DTSV_China, 按last_modified) (默认: True)")
    history_group.add_argument("--skip-history", action='store_true', help="跳过 Defect History 下载")
    history_group.add_argument("--history-max-workers", type=int, default=50, help="并行下载历史线程数 (1-50, 默认: 50，与downloader7一致，可更高视服务器许可而定)")
    history_group.add_argument("--history-output-dir", default="history", help="保存原始Defect History JSON的目录 (默认: history, 与downloader7及history_downloader一致)")
    ...
    if args.fetch_history and not args.skip_history:
        ...
        if not defect_ids_for_history_list:
            ...
            # keep within reasonable bounds; original downloader7 default/cap is 50
